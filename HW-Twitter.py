...
CACHE_FNAME = 'cortez_tweepy_cache.json'
service_url = 'https://stream.twitter.com/1.1/statuses/filter.json?'
...
##		to search for is.  See tweep y_example.py for example code that searches
##      for tweets with a given word or phrase
def get_data(phrase):
	if phrase in CACHE_DICTION:
		print("Fetching info from cache...")
		return CACHE_DICTION[phrase]
	else:
		print("Fetching new info...")
		data = api.search(q=phrase, count=5)

		try:
			CACHE_DICTION[phrase] = data
...
for i in range(3):
	phrase = input("Twitter search: ")
	data = get_data(phrase)
